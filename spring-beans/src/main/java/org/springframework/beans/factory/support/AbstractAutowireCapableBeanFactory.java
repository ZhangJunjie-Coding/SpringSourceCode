...
public abstract class AbstractAutowireCapableBeanFactory extends AbstractBeanFactory
	...
	public Object applyBeanPostProcessorsBeforeInitialization(Object existingBean, String beanName)
		...
		// 遍历该工厂创建的bean的BeanPostProcessor列表
		for (BeanPostProcessor processor : getBeanPostProcessors()) {
			...
			if (current == null) {
				// 直接返回result,中断后续BeanPostProcessor处理
				return result;
			}
			// 让result引用processor的返回结果，使其经过所有BeanPostProcessor对象的后置处理的层层包装
			result = current;
		}
		// 返回经过所有BeanPostProcessor对象的后置处理的层层包装后的result
		return result;
	}
	...
	public Object applyBeanPostProcessorsAfterInitialization(Object existingBean, String beanName)
		...
		for (BeanPostProcessor processor : getBeanPostProcessors()) {
			...
			// 让result引用processor的返回结果，使其经过所有BeanPostProcess对象的后置处理的层层包
			result = current;
		}
	...
	protected Object createBean(String beanName, RootBeanDefinition mbd, @Nullable Object[] args)
		...
		try {
			// 实际创建bean的调用，这里是否会执行doCreateBean取决于,在之前定义的BeanPostProcessor里是否包含了提前创建bean对象的BeanPostProcessor,如果包含则在上面进行创建
	...
	protected Object doCreateBean(String beanName, RootBeanDefinition mbd, @Nullable Object[] args)
		...
		synchronized (mbd.postProcessingLock) {
			if (!mbd.postProcessed) {
				try {
					// 应用MergedBeanDefinitionPostProcessor后置处理器修改合并bean的定义（主要是注解信息，xml配置文件和注解混合使用，需要对beanDefinition进行一个合并）
		...
		// even when triggered by lifecycle interfaces like BeanFactoryA ware.
		...
		if (earlySingletonExposure) {
			...
			// 为避免后期循环依赖，可以在bean初始化完成前将创建实例的ObjectFactory加入工厂 (向三级缓存中放入一个lambda表达式)
...
//			registeredSingletons.add(beanName);
		...
		try {
			// 对bean的属性进行填充，将各个属性值注入，其中可能存在依赖于其他bean的属性，则会递归初始化依赖的bean
			populateBean(beanName, mbd, instanceWrapper);
			// 执行初始化逻辑 (这里对exposedObject赋值，是因为AOP可能会再次创建一个代理对象，下面会对这个对象做一个验证)
		...
		if (earlySingletonExposure) {
			...
			if (earlySingletonReference != null) {
				...
				if (exposedObject == bean) {
					...
					for (String dependentBean : dependentBeans) { // 返回false说明依赖还没实例化好
						if (!removeSingletonIfCreatedForTypeCheckOnly(dependentBean)) {
							actualDependentBeans.add(dependentBean);
						}
					}
					// 因为bean创建后所依赖的bean一定是已经创建的
					// actualDependentBeans不为空则表示当前bean创建后其依赖bean却没有全部创建完，也就是说存在循环依赖
	...
	protected Object getEarlyBeanReference(String beanName, RootBeanDefinition mbd, Object bean) {
		// 默认最终公开对象是bean,通过createBeanInstance创建出来的普通对象
		Object exposedObject = bean;
		...
		if (!mbd.isSynthetic() && hasInstantiationAwareBeanPostProcessors()) {
			...
			for (BeanPostProcessor bp : getBeanPostProcessors()) {
				...
				if (bp instanceof SmartInstantiationAwareBeanPostProcessor) {
					SmartInstantiationAwareBeanPostProcessor ibp = (SmartInstantiationAwareBeanPostProcessor) bp;
					// 让exposedObject经过每个SmartInstantiationAwareBeanPostProcessor的包装 (代理对象)
	...
	protected Object resolveBeforeInstantiation(String beanName, RootBeanDefinition mbd) {
		Object bean = null;
		// 如果beforeInstantiationResolved的值为null或者为true,那么表示尚未被处理，进行后续的处理
		if (!Boolean.FALSE.equals(mbd.beforeInstantiationResolved)) {
			// Make sure bean class is actually resolved at this point.
			// 确认beanClass确实在此处进行处理
			// 判断当前mbd是否是合成的，只有在实现aop的时候synthentic的值才为true，并且是否实现了InstantiationAwareBeanPostProcessor
			if (!mbd.isSynthetic() && hasInstantiationAwareBeanPostProcessors()) {
				// 获取类型
	...
	protected BeanWrapper createBeanInstance(String beanName, RootBeanDefinition mbd, @Nullable Object[] args) {
		...
		// 在使用构造器创建实例后，Spring会将解析过后确定下来的构造器或工厂方法保存在缓存中，避免再次创建相同bean时再次解析,标记下，防止重复创建同一个bean
		// 标记下 防止重复创建同一个bean
		...
		// 使用默认的构造函数构造,如果没有无参构造且存在多个有参构造且没有@Autowired注解构造会报错
	...
	protected Constructor<?>[] determineConstructorsFromBeanPostProcessors(@Nullable Class<?> beanClass, String beanName)
		...
		if (beanClass != null && hasInstantiationAwareBeanPostProcessors()) {
			for (BeanPostProcessor bp : getBeanPostProcessors()) {
				if (bp instanceof SmartInstantiationAwareBeanPostProcessor) {
					// 从SmartInstantiationAwareBeanPostProcessor 判断
					SmartInstantiationAwareBeanPostProcessor ibp = (SmartInstantiationAwareBeanPostProcessor) bp;
	...
	protected void populateBean(String beanName, RootBeanDefinition mbd, @Nullable BeanWrapper bw) {
		...
		// 是否是"synthetic"。一般是指只有A0P相关的pointCut配置或者Advice配置才会将synthetic设置为true
		...
		// 如果自动装配模式为 按名称自动装配bean属性或者 按类型自动装配bean属性
		if (resolvedAutowireMode == AUTOWIRE_BY_NAME || resolvedAutowireMode == AUTOWIRE_BY_TYPE) {
			...
			// 让pvs重新引用newPvs,newPvs此时己经包含了pvs的属性值以及通过AUTOWIRE_BY_NAME,AUTOWIRE_BY_TYPE自动装配所得到的属性值
			pvs = newPvs;
		}
		...
		// 经过筛选的PropertyDescirptor数组，存放着排除忽略的依赖项或忽略项上的定义的属性
		PropertyDescriptor[] filteredPds = null;
		// 如果工厂拥有InstantiationAwareBeanPostProcessor,那么处理对应的流程，主要是对几个注解的赋值工作，包含的两个关键子类时CommonAnnotationBeanPostProcessor，AutowiredAnnotationBeanPostProcessor
		if (hasInstAwareBpps) {
			if (pvs == null) {
			...
			// 遍历工厂内所有后置处理器
			for (BeanPostProcessor bp : getBeanPostProcessors()) {
				// 如果 bp 是InstantiationAwareBeanPostProcessor 的实例
				if (bp instanceof InstantiationAwareBeanPostProcessor) {
					InstantiationAwareBeanPostProcessor ibp = (InstantiationAwareBeanPostProcessor) bp;
					...
					if (pvsToUse == null) {
						// 如果filteredPds为null
						if (filteredPds == null) {
							// mbd,allowCaching:是否允许缓存，默认是允许的。缓存除了可以提高效率以外，还可以保证在并发的情况下，返回的PropertyDescriptor[]永远都是同一份
							// 从bw提取一组经过筛选的PropertyDescriptor,排除忽略的依赖项或忽略项上的定义的属性
							filteredPds = filterPropertyDescriptorsForDependencyCheck(bw, mbd.allowCaching);
						}
						// postProcessPropertyValues:一般进行检查是否所有依赖项都满足，例如基于"Require"注释在bean属性setter,
						// -替换要应用的属性值，通常是通过基于原始的PropertyValues创建一个新的MutablePropertyValue实例，添加或删除特定的值
						// -返回的PropertyValues将应用于bw包装的bean实例的实际属性值（添加PropertyValues:实例到pvs或者设置为null以跳过属性填充）
						// 回到ipd的postProcessPropertyValues方法
						pvsToUse = ibp.postProcessPropertyValues(pvs, filteredPds, bw.getWrappedInstance(), beanName);
						// 如果pvsToUse为null，则跳过属性填充
						if (pvsToUse == null) {
							return;
						}
					}
	...
	/** 通过bw的propertyDescriptors属性名，查找出对应的Bean对象，将其添加到Pvs中
	...
	protected void autowireByType(
		...
		// 获取bw中有setter方法 && 非常简单类型属性 && mbd的PropertyValues中没有该pd的属性名的 PropertyDescriptor 属性名数组
		String[] propertyNames = unsatisfiedNonSimpleProperties(mbd, bw);
		// 遍历属性名数组
		for (String propertyName : propertyNames) {
			try {
				// PropertyDescriptor: 表示JavaBean类通过存储器导出一个属性
				// 从bw中获取PropertyName对应的PropertyDescritpor
				PropertyDescriptor pd = bw.getPropertyDescriptor(propertyName);
				// Don't try autowiring by type for type Object: never makes sense,
				// even if it technically is a unsatisfied, non-simple property.
				// 不要尝试按类型自动装配对象，永远是有意义的，即使他在技术上是一个不满意，复杂属性
				// 如果pd的属性值类型不是Object
	...
	protected String[] unsatisfiedNonSimpleProperties(AbstractBeanDefinition mbd, BeanWrapper bw) {
		//TreeSet:TreeSet底层是二叉树，可以对对象元素进行排序，但是自定义类需要实现comparable接口，重写compareTo()方法。
	...
	protected void applyPropertyValues(String beanName, BeanDefinition mbd, BeanWrapper bw, PropertyValues pvs) {
		...
		// 获取用户自定义类型转换器 如果有自定义的使用自定义的，如果没有则使用beanWrapper对象，因为默认的beanWrapper实现了TypeConverter接口
		TypeConverter converter = getCustomTypeConverter();
		...
		for (PropertyValue pv : original) {
			...
			// 如果属性没有被解析过
				...
				if (originalValue == AutowiredPropertyMarker.INSTANCE) {
					// 获取propertyName在bw中setter方法
					Method writeMethod = bw.getPropertyDescriptor(propertyName).getWriteMethod();
					// 如果setter方法为null
					if (writeMethod == null) {
						// 抛出非法参数异常:自动装配标记没有写方法
						throw new IllegalArgumentException("Autowire marker for property without write method: " + pv);
					}
					// 将writeMethod封装到DependencyDescriptor对象
				...
				//可转换标记：propertyName是否bw中的可写属性&&propertyName不是表示索引属性或嵌套属性（如果propertyName中有'.'丨|'['就认为是索引属性或嵌套属性
				...
				if (resolvedValue == originalValue) {
					...
					// convertedValue不是Collection对象或数组
		...
		try {
			// 按原样使用deepCopy构造一个新的MutablePropertyValues对象然后设置到bw中以对bw属性值更新
	...
	protected Object initializeBean(String beanName, Object bean, @Nullable RootBeanDefinition mbd) {
		if (System.getSecurityManager() != null) {
			...
			// 对特殊的bean处理Aware接口，这里只调用BeanNameAware、BeanClassLoaderAware、BeanFactoryAware接口 其他的接口不处理（在CommonAnnotationBeanPostProcessor中处理）
			invokeAwareMethods(beanName, bean);
		}

		Object wrappedBean = bean;
		//如果mdb不为null || mbd不是"synthetic"。一般是指只有AoP相关的pointCut配置或者Advice配置才会将synthetic设置为true
		...
		if (mbd == null || !mbd.isSynthetic()) {
			// 将BeanPostProcessor应用到给定的现有的bean实例  调用它们的postProcessAfterInitialization方法。返 回的Bean实例可能是原始bean包装器(AOP代理是在这里生成的)
	...
	protected void invokeInitMethods(String beanName, Object bean, @Nullable RootBeanDefinition mbd)
		...
		// InitializingBean:当Bean的所有属性都被BeanFactory设置好后，Bean需要执行相应的接口：例如执行自定义初始化，或者仅仅是检查所有强制属性是否己经设置好。bean是InitializingBean实例标记。
