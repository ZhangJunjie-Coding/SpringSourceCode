...
public abstract class AbstractPlatformTransactionManager implements PlatformTransactionManager, Serializable {
	...
	private TransactionStatus handleExistingTransaction(
		...
		/**
		 * 当前的事务属性状态是 PROPAGATION_REQUIRES_NEW 表示需要开启一个事务状态
		 */
	...
	/**
	 * 通过给定的参数创建一个事务状态，有一个非常关键的参数是newTransaction，用于判断是否是新连接，如果事务不存在，那么肯定是true，如果存在，就需要根据传播特性来决定是否是false了
	 ...
	 * 最后限据条件执行数据清除，线程的私有资源解绑，重置连接自动提交，隔离级别，是否只读，释放链接，恢复挂起事务等
	 ...
	 * 然后判断是否设置了保存点，比如NESTED会设置，要先回滚到保存点。如果状态是新的事务，那就进行回滚，加果不是新的，就设置一个回滚标记，
