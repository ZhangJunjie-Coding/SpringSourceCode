...
public class DataSourceTransactionManager extends AbstractPlatformTransactionManager
	...
	protected Object doGetTransaction() {
		 ...
		 * 用来保存当前事务的信息，我们第一次从这里去线程变量中获取 事务链接持有器对象 通过数据源为key去获取
		 * 由于第一次进来开始事务 我们的事务同步管理器中没有被存放，所以此时获取出来的connectionHolder为null
		 */
	...
	protected void doBegin(Object transaction, TransactionDefinition definition) {
		...
		try {
			...
			// 绑定我们的数据源和连接到我们的同步管理器上，把数据源作为key，数据库连接作为value设置到线程变量中
	...
	/**
	 * 此方法做清除连接相关操作，比如重置自动提交，只读属性，解绑数据源，释放连接，清除连接持有器属性
	 * @param transaction the transaction object returned by {@code doGetTransaction}
	 */
	@Override
