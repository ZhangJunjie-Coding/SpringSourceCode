...
abstract public class AbstractClassGenerator<T> implements ClassGenerator {
	...
	protected Object create(Object key) {
		try {
			...
			// 当前类加载器对应的缓存 缓存key为类加载器，缓存的value为ClassLoaderData，可以理解为一个缓存对象，只不过此缓存对象中包含的是具体的业务逻辑处理过程，有两个function函数式接口，一个返回gen.key，对应的名称为GET_KEY，还有一个是为了创建具体的class,名字叫做load
			...
			if (data == null) {
				synchronized (AbstractClassGenerator.class) {
					...
					if (data == null) {
						...
						// 新建一个当前加载器对应的ClassLoaderData并加载到缓存中，但ClassLoaderData中此时还没有数据
